    ...
    const doAutoLogin = async () => {
      ...
      try {
        ...
        if (result?.tokens?.accessToken) {
          ...
          success = true;
          // redirect to blog dashboard directly (skips the / → /blog redirect chain
          // which can cause useEffect race conditions in React 19)
          router.replace('/blog');
        }
      } catch {
        ...
        // On success, the router.replace('/blog') above handles navigation.
        if (!success) {
