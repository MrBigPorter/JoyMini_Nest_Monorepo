  ...
  const localize = useCallback(
    (fieldName: keyof T) => {
      ...
      if (fieldKey === 'name' || fieldKey === 'description') {
        console.log('DEBUG useLocalizedForm - localize() called:', {
          fieldKey,
          rawValue,
          rawValueType: typeof rawValue,
          locale,
          safeValue,
          safeValueType: typeof safeValue,
          storageRef: storageRef.current[fieldKey],
          initialized: initializedRef.current.has(fieldKey),
        });
      }
