...
export function ArticleCard({
  ...
  return (
    <div className="group relative bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-700 p-6 transition-all hover:shadow-md hover:border-primary/20 transform duration-150 ease-in-out">
      {/* 收藏按钮 - 右上角，始终显示 */}
      {showBookmarkButton && (
      ...
      <Link href={`/articles/${article.slug}`} className="block">
        <div className="space-y-3">
          {/* 标题 */}
          <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-200 group-hover:text-primary-600 dark:group-hover:text-primary-500 transition-colors line-clamp-2 pr-12">
            {article.title}
          </h3>
