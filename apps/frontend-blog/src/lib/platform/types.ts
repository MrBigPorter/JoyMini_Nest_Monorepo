...
/** 平台QueryKey类型（添加平台前缀） */
export type PlatformQueryKey = readonly [PlatformType, ...QueryKey];
...
export interface IPlatformAdapter {
  ...
  query: {
    /** 为queryKey添加平台/语言前缀 */
    buildQueryKey: (baseKey: QueryKey) => PlatformQueryKey;
    ...
    /** 获取默认的查询选项 */
    getDefaultQueryOptions: () => Partial<PlatformQueryOptions<unknown>>;
  };
...
/** 平台感知的Mutation选项 */
export interface PlatformMutationOptions<
  TData = unknown,
  TVariables = void,
  TContext = unknown,
> {
...
export interface PlatformInfiniteQueryOptions<
  T = unknown,
  TPageParam = unknown,
> {
  /** 基础queryKey */
...
/** 平台事件监听器 */
export type PlatformEventListener = (event: PlatformEvent, data?: EventData) => void;
