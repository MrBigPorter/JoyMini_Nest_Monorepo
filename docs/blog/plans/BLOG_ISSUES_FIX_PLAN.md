...
> - 弹窗状态残留问题已解决
> - 前端博客全面进度分析完成
> - **国际化架构改造完成**：从API动态获取改为本地文件扫描方案
> - **语言标识符统一**：zh-CN → zh，修复路由配置问题
> - **TypeScript类型问题修复**：确保编译通过
> - **路由404错误修复**：/zh/ 和 /en/ 路由现在可正常访问
...
4. **国际化架构统一问题** - frontend-blog 使用硬编码语言列表，未读取系统配置
...
**问题描述**: frontend-blog 使用硬编码语言列表 `['zh-CN', 'en']`，未读取系统配置中启用的语言列表。
...
1. **语言标识符不一致**: frontend-blog 使用 `zh-CN`，admin-next 使用 `zh`
2. **配置来源不同**: frontend-blog 使用硬编码配置，admin-next 从系统配置读取
3. **缺少公共API**: frontend-blog 无法获取系统启用的语言列表

**解决方案**:

1. **统一语言标识符**: 将 frontend-blog 的 `zh-CN` 改为 `zh`
2. **创建公共API**: 添加 `/v1/public/system-config/locales` 端点
3. **动态语言配置**: frontend-blog 从系统配置读取启用的语言列表
4. **统一LanguageProvider**: 所有前端应用使用相同的语言上下文

**实施步骤**:

1. **阶段一**: 分析现有API接口权限要求
2. **阶段二**: 创建 frontend-blog 的 `useAvailableLocales` hook
3. **阶段三**: 更新 frontend-blog 配置使用动态语言列表
4. **阶段四**: 测试验证和回退机制
