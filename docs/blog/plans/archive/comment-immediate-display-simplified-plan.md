...
```typescript
// 方案A: 使用现有评论列表API（推荐）
async function checkCommentStatus(
  commentId: string,
  ...
  try {
    ...
    // 检查评论是否在列表中
    const commentExists = comments.items.some(
...
const Comment = ({ comment, depth = 0, articleId }: CommentProps) => {
  ...
  return (
    <div
      className={`comment-item ${commentStatus === "rejected" ? "removing" : ""}`}
    >
      {/* 现有评论内容 */}
      {renderStatusBadge()}
    </div>
...
_基于原始计划 `comment-immediate-display-plan.md` 的简化版本，聚焦核心问题解决。_
