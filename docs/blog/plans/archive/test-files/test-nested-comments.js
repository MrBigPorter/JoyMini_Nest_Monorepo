      ...
      res.on('end', () => {
        try {
          ...
          if (result.data?.items && result.data.items.length > 0) {
            ...
            console.log(`   第一个评论是否有children字段: ${'children' in firstComment}`);
            console.log(`   第一个评论的children数量: ${firstComment.children?.length || 0}`);
            
            // 检查是否有嵌套的子评论
            if (result.data.items.length > 1) {
              const secondComment = result.data.items[1];
              console.log(`   第二个评论的children数量: ${secondComment.children?.length || 0}`);
              
              if (secondComment.children && secondComment.children.length > 0) {
                const firstChild = secondComment.children[0];
                console.log(`   第一个子评论是否有children字段: ${'children' in firstChild}`);
                console.log(`   第一个子评论的children数量: ${firstChild.children?.length || 0}`);
              }
            }
          }
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = http.request(options, (res) => {
      ...
      res.on('end', () => {
        ...
        // 检查页面是否包含评论相关的内容
        if (data.includes('comment') || data.includes('Comment') || data.includes('评论')) {
...
async function runTests() {
  ...
  try {
    ...
    console.log('\n🔍 下一步:');
    console.log('   1. 打开浏览器访问 http://localhost:4001/zh/articles/xss-attack-defense-complete-guide');
