...
export function localizedStringSchema<T extends z.ZodTypeAny>(valueSchema: T) {
  return z.record(z.union([valueSchema, z.undefined()]));
}
...
export function getLocalizedValue<T>(
  ...
  // 回退到默认语言
  if (fallback !== locale && field[fallback] !== undefined && field[fallback] !== null) {
 ...
 * @param enValue 英文值
 */
export function migrateFromLegacyFields<T>(zhValue: T | null | undefined, enValue: T | null | undefined): LocalizedString<T> {
