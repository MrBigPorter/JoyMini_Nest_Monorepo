...
### 可选：不使用单独的 Module，直接在 AppModule 里配置

也可以直接在 `AppModule` 的 `imports` 里加上 `GraphQLModule.forRoot(...)`，不需要新建文件。两种都可以，选你喜欢的。
...
| 检查点 | 结果 |
|--------|------|
| `yarn workspace @lucky/api add ...` | ✅ 依赖安装成功 |
| `apps/api/src/graphql/graphql.module.ts` | ✅ 文件创建 |
| `apps/api/src/app.module.ts` 注册 | ✅ 模块注册 |
| `http://localhost:3000/graphql` | ✅ 显示 Playground 页面 |

---

**完成后告诉我，我写 Step 2：创建第一个 GraphQL Query（文章列表）**
