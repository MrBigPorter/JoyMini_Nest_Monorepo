...
async function diagnoseLocalizedFields() {
  ...
  try {
    ...
    for (const article of articles) {
      ...
      console.log(`  titleLocalized: ${article.titleLocalized ? JSON.stringify(article.titleLocalized) : '空'}`);
      
      if (article.titleLocalized) {
        const titleLocalized = article.titleLocalized;
        console.log(`  titleLocalized 包含语言: ${Object.keys(titleLocalized).join(', ')}`);
      ...
      console.log(`  excerptLocalized: ${article.excerptLocalized ? JSON.stringify(article.excerptLocalized) : '空'}`);
      
      if (article.excerptLocalized) {
        const excerptLocalized = article.excerptLocalized;
        console.log(`  excerptLocalized 包含语言: ${Object.keys(excerptLocalized).join(', ')}`);
      ...
      console.log(`  contentLocalized: ${article.contentLocalized ? '有值' : '空'}`);
      ...
      if (article.contentLocalized) {
        ...
        if (typeof contentLocalized === 'object') {
          console.log(`  contentLocalized 包含语言: ${Object.keys(contentLocalized).join(', ')}`);
          if (!contentLocalized.zh && article.content) {
            console.log(`  ❌ 问题: contentLocalized 缺少中文，但 content 有值`);
          }
        }
      ...
      console.log(`  contentMdLocalized: ${article.contentMdLocalized ? '有值' : '空'}`);
      ...
      if (article.contentMdLocalized) {
        ...
        if (typeof contentMdLocalized === 'object') {
          console.log(`  contentMdLocalized 包含语言: ${Object.keys(contentMdLocalized).join(', ')}`);
          if (!contentMdLocalized.zh && article.contentMd) {
            console.log(`  ❌ 问题: contentMdLocalized 缺少中文，但 contentMd 有值`);
          }
        }
