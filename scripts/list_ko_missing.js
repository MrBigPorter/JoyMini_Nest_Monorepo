...
const koBak = fs.existsSync(koBakPath) ? JSON.parse(fs.readFileSync(koBakPath,'utf8')) : null;
...
if(en.blogCard){
  Object.keys(en.blogCard).forEach(k => {
    if(!koBakFlat || !koBakFlat.blogCard || !(k in koBakFlat.blogCard)) missingInBak.push('blogCard.'+k);
  });
}
...
if(en.blogCard){
  Object.keys(en.blogCard).forEach(k => {
    ...
    const koV = (koCurrFlat.blogCard && (koCurrFlat.blogCard[k] !== undefined)) ? koCurrFlat.blogCard[k] : null;
    if(koV !== null && koV === enV) filledWithEnglish.push('blogCard.'+k);
  });
}
fs.writeFileSync(p + '/ko.missing_keys.txt', missingInBak.join('\n'));
fs.writeFileSync(p + '/ko.filled_with_english.txt', filledWithEnglish.join('\n'));
console.log('missing count (original ko):', missingInBak.length);
console.log('filled-with-english count:', filledWithEnglish.length);
console.log('Files written:', p + '/ko.missing_keys.txt', p + '/ko.filled_with_english.txt');
