...
function checkFileContent(filePath, keywords) {
  try {
    ...
    console.log(`❌ ${path.relative(PROJECT_ROOT, filePath)} 读取失败: ${error.message}`);
    return false;
  }
}
...
function runTypeCheck() {
  try {
    ...
    const capacitorErrors = lines.filter(line => line.includes('@capacitor/preferences'));
    ...
    if (otherErrors.length === 0 && capacitorErrors.length > 0) {
      console.log('⚠️ 只有 Capacitor 相关类型错误（预期中，因为未安装 Capacitor）');
      console.log('Capacitor 错误详情:');
      capacitorErrors.slice(0, 3).forEach(err => console.log(`  ${err}`));
      if (capacitorErrors.length > 3) console.log(`  ...还有 ${capacitorErrors.length - 3} 个错误`);
      return true; // 视为通过，因为 Capacitor 是可选的
    } else {
      console.log('其他类型错误:');
      otherErrors.slice(0, 5).forEach(err => console.log(`  ${err}`));
      if (otherErrors.length > 5) console.log(`  ...还有 ${otherErrors.length - 5} 个错误`);
      return false;
    }
  }
}
...
async function runTests() {
  ...
  const filesToCheck = [
    // 架构文档
    {
      path: path.join(PROJECT_ROOT, 'docs/nextjs/AUTH_ARCHITECTURE_ZERO_FLICKER.md'),
    ...
    // 更新版 useAuth hook
    {
      ...
      keywords: ['store.isAuthenticated()', 'store._synced', 'store.syncFromStorage'],
    },
    // 新的 ProtectedRouteV2 组件
    {
      path: path.join(FRONTEND_BLOG_DIR, 'components/auth/ProtectedRouteV2.tsx'),
      keywords: ['ProtectedRouteV2', 'effectiveAuth', 'ssrAuth', 'platform.isSSR'],
    },
    // 更新后的 bookmarks 页面
    {
