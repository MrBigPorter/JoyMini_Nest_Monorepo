...
// 1. 检查类型声明文件
const capacitorTypesPath = path.join(__dirname, '../apps/frontend-blog/src/types/capacitor.d.ts');
console.log('1. 检查类型声明文件...');
if (fs.existsSync(capacitorTypesPath)) {
  const content = fs.readFileSync(capacitorTypesPath, 'utf8');
  const hasModuleDeclaration = content.includes("declare module '@capacitor/preferences'");
  const hasPreferencesExport = content.includes('export const Preferences: PreferencesPlugin');
...
// 2. 检查 platform.ts 修复
const platformPath = path.join(__dirname, '../apps/frontend-blog/src/lib/utils/platform.ts');
console.log('\n2. 检查 platform.ts 修复...');
if (fs.existsSync(platformPath)) {
  ...
  const checks = {
    ...
    '安全的动态导入': content.includes("const { Preferences } = await import('@capacitor/preferences')"),
    'try-catch 错误处理': content.includes('try {') && content.includes('} catch (error)'),
    'fallback 到 localStorage': content.includes('localStorage.getItem(key)') || content.includes('localStorage.setItem'),
  ...
  // 检查是否还有可能导致SSR错误的代码
  const hasUnsafeImport = content.includes("import('@capacitor/preferences')") && 
...
const packageJsonPath = path.join(__dirname, '../apps/frontend-blog/package.json');
console.log('\n3. 检查 package.json 依赖...');
if (fs.existsSync(packageJsonPath)) {
  const packageJson = JSON.parse(fs.readFileSync(packageJsonPath, 'utf8'));
  const hasCapacitorDependency = packageJson.dependencies && 
                                 packageJson.dependencies['@capacitor/preferences'];
  
  console.log(`   📦 @capacitor/preferences 依赖: ${hasCapacitorDependency ? '已安装' : '未安装（可选依赖）'}`);
...
console.log('     预期: 尝试导入 @capacitor/preferences，失败时fallback到 localStorage');
...
if (fs.existsSync(platformPath)) {
  ...
  const unsafePatterns = [
    /await import\('@capacitor\/preferences'\)/g,
  ];
  ...
  if (unsafeCount > 0 && safeCount < unsafeCount * 2) {
    issues.push(`⚠️  安全防护可能不足: ${unsafeCount}个动态导入，${safeCount}个安全防护`);
  }
}
