...
if (fs.existsSync(hookPath)) {
  ...
  const checks = {
    ...
    useFrontendCategories包含locale:
      content.includes("useFrontendCategories() {") &&
      content.includes("queryKey: ['frontendCategories', locale]"),
    useFrontendCategoryBySlug包含locale:
      content.includes("useFrontendCategoryBySlug(") &&
      content.includes("queryKey: ['frontendCategory', slug, locale"),
    useFrontendTags包含locale:
      content.includes("useFrontendTags() {") &&
      content.includes("queryKey: ['frontendTags', locale]"),
    useFrontendTagBySlug包含locale:
      content.includes("useFrontendTagBySlug(") &&
      content.includes("queryKey: ['frontendTag', slug, locale"),
  };
...
if (fs.existsSync(categoriesPagePath)) {
  ...
  const checks = {
    ...
    "从next-intl导入useTranslations": content.includes(
      "import { useTranslations } from 'next-intl'",
    ),
    正确使用国际化:
      content.includes("t('categories.title')") ||
      content.includes('t("categories.title")'),
  };
...
// 3. 检查分类详情页面
const categoryDetailPagePath = path.join(
  __dirname,
...
if (fs.existsSync(categoryDetailPagePath)) {
  ...
  // 检查关键修复点
  const checks = {
    使用useFrontendCategoryBySlug: content.includes(
      "useFrontendCategoryBySlug",
    ),
    从路由参数获取slug: content.includes("params.slug"),
    正确使用国际化:
      content.includes("t('categories.notFound')") ||
      content.includes('t("categories.notFound")'),
  };
