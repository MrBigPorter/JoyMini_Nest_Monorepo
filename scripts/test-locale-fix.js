...
// 1. 检查HTTP客户端修复
const httpPath = path.join(__dirname, '../apps/frontend-blog/src/lib/api/http.ts');
...
if (fs.existsSync(httpPath)) {
  ...
  // 检查关键修复点
  const checks = {
    '添加lang查询参数': content.includes('config.params = {') && content.includes('lang: lang'),
    '保留Accept-Language头部': content.includes("config.headers['Accept-Language'] = lang"),
    '符合后端优先级': content.includes('后端 LanguageService.resolveLanguage() 优先使用查询参数'),
  };
...
const articlesHookPath = path.join(__dirname, '../apps/frontend-blog/src/lib/hooks/useArticlesInfiniteQuery.ts');
console.log('\n2. 检查useArticlesInfiniteQuery修复...');
if (fs.existsSync(articlesHookPath)) {
  ...
  const checks = {
    ...
    '获取locale参数': content.includes('const locale = (params.locale as string)') || content.includes('const locale = params.locale as string'),
    'queryKey包含locale': (content.includes('locale }') || content.includes('locale,')) && content.includes('queryKey'),
  };
...
const hooksToCheck = [
  'useBookmarks.ts',
  'useBookmarksInfiniteQuery.ts',
];

hooksToCheck.forEach(hookFile => {
  const hookPath = path.join(__dirname, '../apps/frontend-blog/src/lib/hooks', hookFile);
...
if (fs.existsSync(articlesHookPath)) {
  const content = fs.readFileSync(articlesHookPath, 'utf8');
  const hasLocaleInQueryKey = (content.includes('locale }') || content.includes('locale,')) && content.includes('queryKey');
