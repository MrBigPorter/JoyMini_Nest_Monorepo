  ...
  try {
    ...
    if (jobsData.failed && jobsData.failed.length > 0) {
      ...
      jobsData.failed.slice(0, 3).forEach((job, index) => {
        ...
        console.log(`      时间戳: ${new Date(job.timestamp).toLocaleString()}`);
      });
    ...
    if (jobsData.active && jobsData.active.length > 0) {
      ...
      jobsData.active.slice(0, 3).forEach((job, index) => {
        ...
        console.log(`      开始时间: ${job.processedOn ? new Date(job.processedOn).toLocaleString() : '未知'}`);
      });
    }
    ...
    console.log('      - 查看 apps/api/src/blog/blog.service.ts 中的 getTranslationJobs() 方法');
