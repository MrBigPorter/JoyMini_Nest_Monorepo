...
const testCases = [
  ...
  {
    input: "使用NestJS构建API",
    expected: "使用NestJS构建API",
    description: "混合内容1",
  },
  {
    input: "Docker容器化部署",
    expected: "Docker容器化部署",
    description: "混合内容2",
  },
  {
    input: "PostgreSQL数据库优化",
    expected: "PostgreSQL数据库优化",
    description: "混合内容3",
  },
...
async function mockTranslateText(text, targetLang) {
  ...
  // 检查是否包含技术术语
  const containsTechTerm = techTerms.some(
    (term) =>
  ...
  for (const testCase of testCases) {
    ...
    try {
      ...
      if (isTechTerm) {
        ...
        // 普通内容应该被翻译（至少应该改变）
        testPassed =
          (result !== input && result.includes("Security")) ||
          result.includes("Performance") ||
      ...
      if (testPassed) {
        ...
        console.log(`   期望: "${expected}"`);
        console.log(
          `   问题: ${isTechTerm ? "技术术语被翻译了" : "普通内容未被正确翻译"}\n`,
        );
        failed++;
      }
    } catch (error) {
...
async function runIntegrationTest() {
  ...
  const envVars = ["GOOGLE_VISION_CREDENTIALS", "GOOGLE_PROJECT_ID"];
