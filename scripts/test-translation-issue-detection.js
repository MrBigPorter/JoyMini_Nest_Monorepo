...
mockArticles.forEach((article, index) => {
  ...
  if (issues.length > 0) {
    ...
    issues.forEach((issue, issueIndex) => {
      console.log(
        `  ${issueIndex + 1}. [${issue.severity}] ${issue.issueType}: ${issue.description}`,
      );
    });
...
console.log(
  `  - 发现问题文章数: ${mockArticles.filter((a) => detectArticleTranslationIssues(a, "en").length > 0).length}`,
);
console.log(`  - 总问题数: ${totalIssues}`);

// 测试批量修复逻辑
console.log("\n🛠️ 测试批量修复逻辑:");
const problematicArticles = mockArticles.filter(
  (a) => detectArticleTranslationIssues(a, "en").length > 0,
);
console.log(`  - 需要修复的文章数: ${problematicArticles.length}`);
console.log(
  `  - 文章ID列表: ${problematicArticles.map((a) => a.id).join(", ")}`,
);
...
console.log("\n📁 已更新的文件:");
console.log(
  "  - apps/api/src/blog/blog.service.ts (添加问题检测和批量修复方法)",
);
console.log("  - apps/api/src/blog/blog.controller.ts (添加API端点)");
console.log("  - apps/admin-next/src/api/index.ts (添加API客户端方法)");
console.log(
  "  - apps/admin-next/src/views/blog/BlogTranslationProgress.tsx (添加前端UI)",
);
