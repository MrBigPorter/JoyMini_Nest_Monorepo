// 测试语言检测工具
const { LanguageDetectionService } = require('./apps/api/dist/common/services/language-detection.service');
...
testCases.forEach((testCase, index) => {
  ...
  console.log(`     检测: ${result.language} (置信度: ${result.confidence.toFixed(2)})`);
...
completenessTests.forEach((test, index) => {
  ...
  console.log(`     问题: ${result.issues.length > 0 ? result.issues.join(', ') : '无'}`);
});
...
fieldTests.forEach((test, index) => {
  ...
  console.log(`  ${index + 1}. 源: "${test.source}" -> 目标: "${test.target}" (${test.language})`);
  console.log(`     已翻译: ${result.translated} (置信度: ${result.confidence.toFixed(2)})`);
...
residualTests.forEach((test, index) => {
  ...
  console.log(`  ${index + 1}. 源语言: ${test.sourceLang}, 目标语言: ${test.targetLang}`);
  console.log(`     文本: "${test.targetText}"`);
  console.log(`     检测到: ${result.length > 0 ? result.join(', ') : '无'}`);
  console.log(`     预期: ${test.expected.length > 0 ? test.expected.join(', ') : '无'} ${passed ? '✅' : '❌'}`);
});
